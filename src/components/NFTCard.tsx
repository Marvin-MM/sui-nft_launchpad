  ...
  return (
    ...
    >
      <div className="relative aspect-square overflow-hidden">
        ...
        {nft.staked && (
          <div className="absolute top-4 right-4 px-3 py-1 rounded-full bg-accent-primary text-white text-[10px] font-black italic tracking-widest uppercase flex items-center gap-1 shadow-lg shadow-accent-primary/50">
        ...
        <div className="absolute inset-0 bg-linear-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
          <div className="grid grid-cols-2 gap-2">
            <button 
              ...
              className="px-4 py-2 bg-white/10 backdrop-blur-md rounded-xl text-[10px] font-black italic tracking-widest uppercase hover:bg-white/20 transition-all flex items-center justify-center gap-2"
            >
            ...
            <button 
              ...
              className="px-4 py-2 bg-accent-primary/80 backdrop-blur-md rounded-xl text-[10px] font-black italic tracking-widest uppercase hover:bg-accent-primary transition-all flex items-center justify-center gap-2"
            >
      ...
      <div className="p-6 space-y-4">
        <div className="flex justify-between items-start">
          <div className="space-y-1">
            <h3 className="text-xl font-black italic tracking-tight">{nft.name}</h3>
            ...
            <div className="absolute right-0 top-full mt-2 w-48 glass-card p-2 opacity-0 group-hover/menu:opacity-100 pointer-events-none group-hover/menu:pointer-events-auto transition-all duration-200 z-20 shadow-2xl border-white/10">
              {[
                ...
                <button
                  ...
                  className="w-full px-4 py-2 rounded-lg text-xs font-black italic tracking-widest uppercase hover:bg-white/5 flex items-center gap-3 text-white/60 hover:text-white transition-all"
                >
        ...
        <div className="flex flex-wrap gap-2">
          {nft.traits.slice(0, 3).map((trait) => (
            <div key={trait.key} className="px-2 py-1 rounded bg-white/5 border border-white/10 text-[10px] font-black italic tracking-widest uppercase text-white/40">
              {trait.key}: <span className="text-white/80">{trait.value}</span>
            </div>
          ))}
          {nft.traits.length > 3 && (
            <div className="px-2 py-1 rounded bg-white/5 border border-white/10 text-[10px] font-black italic tracking-widest uppercase text-white/40">
